            ...
            <form action="{{ route('recipes.update', $recipe) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-4">
                    ...
                    <div class="items-start md:grid md:grid-cols-9 md:space-x-6">
                        ...
                        <div class="w-full md:col-span-4 grid grid-cols-1 md:grid-cols-12">
                            ...
                            <div
                                ...
                                x-data x-init="
                                    FilePond.create($refs.input, ja_JA);
                                    FilePond.setOptions({
                                        server: {
                                            url: '/upload',
                                            headers: {
                    ...
                    <div class="space-y-2 md:flex md:space-y-0 md:space-x-3">
                        <div>
                            <input type="checkbox" name="adults" id="adults" value="1"
                                {{ $recipe->adults ? 'checked' : '' }}
                                class="rounded text-green-700 focus:outline-none focus:ring-1 focus:border-green-500 focus:ring-green-500 dark:bg-gray-900 dark:border-transparent dark:text-gray-200 dark:focus:bg-green-700">
                        ...
                        <div class="w-full md:col-span-4 md:pt-1 text-2xl">
                            @foreach ($allAllergens as $allergen)
                                <div class="inline cursor-pointer" x-data="{ level: {{ array_key_exists($allergen->id, $allergens) ? "'" . $allergens[$allergen->id] . "'" : "'no'" }} }">
                                    <span x-on:click="level = changeLevel(level)"
                                        x-bind:class="{
                    ...
                    <div class="md:space-y-2">
                        ...
                        <div>
                            <textarea name="instruction" id="instruction"
                                class="border-1 border-gray-100 shadow bg-opacity-20 rounded-lg placeholder-gray-500 w-full h-64 focus:outline-none focus:ring-1 focus:border-green-500 focus:ring-green-500 dark:bg-gray-900 dark:border-transparent dark:text-gray-200">{!! old('instruction', $recipe->instruction) !!}</textarea>
                        </div>
                    </div>
                    <div>
                        <button type="submit"
